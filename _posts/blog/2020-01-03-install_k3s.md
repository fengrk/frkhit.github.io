...
安装 k3s， 推荐使用官方提供的安装脚本在线安装：

```
curl -sfL https://get.k3s.io | sh -
```

但由于安装过程中，需要从 github 拉取可执行文件和离线镜像包，安装耗时非常长。

加速方式， 使用提前下载好的可执行文件和离线镜像包，可加速 k3s 安装。

编辑并执行 `k3s_offline_install.sh`(代码基于 `https://get.k3s.io`)即可实现安装加速。
...
setup_binary() {
    ...
    # pull image
    [[ $(docker images frkhit/docker-practice:k3s-file | wc -l) -ne "2" ]] && docker pull frkhit/docker-practice:k3s-file

    # run docker
    [[ $(docker ps -f name=k3s-file | wc -l) -ne "2" ]] && docker run -d --name k3s-file frkhit/docker-practice:k3s-file tail -f /dev/null

    # cp file
    docker cp k3s-file:/k3s ${TMP_BIN}
    docker cp k3s-file:/k3s-airgap-images-amd64.tar ${TMP_DIR}/
    mkdir -p /var/lib/rancher/k3s/agent/images/
    mv ${TMP_DIR}/k3s-airgap-images-amd64.tar /var/lib/rancher/k3s/agent/images/
